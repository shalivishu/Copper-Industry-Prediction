...
class prediction:
    ...
    def regression():
        ...
        with st.form('Regression'):
            ...
            with col3:
                ...
                width = st.text_input(label='Width (Min: 1  Max: 1000000)')
        ...
        if button:
            if (validate_input(quantity_log) and validate_input(customer)):
                # load the regression pickle model
                with open(r'C:\Users\shali\ML Folder\DecisionTreeRegressor.pkl', 'rb') as f:
                    model = pickle.load(f)
    ...
    def classification():
        ...
        # user entered the all input values and click the button
        if button:
            if (validate_input(quantity_log) and validate_input(customer) and validate_input(selling_price_log)):
                
                # load the classification pickle model
                with open(r'C:\Users\shali\ML Folder\extratrees_model.pkl', 'rb') as f:
                    model = pickle.load(f)
...
with tab2:
    ...
    try:
        ...
        if status == 1:
            ...
            # apply custom css style for prediction text
            style_prediction()
            st.markdown(f'### <div class="center-text">YOU WON</div>', unsafe_allow_html=True)
            st.snow()
            ...
            # apply custom css style for prediction text
            style_prediction()
            st.markdown(f'### <div class="center-text">YOU LOST</div>', unsafe_allow_html=True)
            st.snow()
